...
const handleCreate = (comp) => {
  ...
  traceDir(path.resolve(__dirname, COMP_TEMPLATE_FOLDER), {
    dirCallback: (dpath) => {
      const dirName = dpath.slice(dpath.indexOf(COMP_TEMPLATE_FOLDER) + 14);
      fs.mkdirSync(
        path.resolve(workplaceRoot, "src/components", componentName, dirName)
      );
    },
    fileCallback: (fpath) => {
      ...
      fs.writeFileSync(
        path.resolve(
          path.resolve(workplaceRoot, "src/components", componentName, fileName)
        ),
        template(options)
      );
    },
