...
console.log(chalk.black.bgGreen('            欢迎使用nice-ui组件移除工具              '));
...
inquirer.prompt([
  {
    ...
    validate: function(input) {
      ...
      if (!input) {
        ...
        if (!fs.existsSync(path.resolve(workplaceRoot, 'src/components', camel2line(input)))) {
          done('组件不存在，请重新输入');
          return;
        }
      }
...
const handleAnswers = (answers) => {
  inquirer.prompt([
    ...
    if (isCorrent === '是') {
      ...
      console.log(chalk.black.bgGreen(`            组件【${answers.componentName}】已移除              `));
    }
    process.exit();
  });
}
